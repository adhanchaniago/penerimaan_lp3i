...
class M_Peserta extends CI_Model
{
	...
	public function get_pengumuman_terima($tahun,$id)
	{
		$this->db->select("pendaftar.*, peserta.ID, peserta.TOTAL_NILAI, peserta.KETERANGAN, peserta.KEPUTUSAN, peserta.CATATAN");
		$this->db->from("peserta");
		$this->db->join("pendaftar", "peserta.no_pendaftaran = pendaftar.no_pendaftaran", "right");
		$this->db->where('peserta.ID',$id);
		$this->db->like('pendaftar.TANGGAL_DAFTAR', $tahun, 'after');
		$this->db->order_by('peserta.KEPUTUSAN','DESC');
		return $this->db->get()->result();
	}
	...
	public function join_jurusan(array $cond)
	{
		$this->db->select("*");
		$this->db->from("peserta");
		$this->db->join("jurusan", "jurusan.ID_JURUSAN = peserta.KEPUTUSAN ","inner");
		if(count($cond) > 0)
			$this->db->where($cond);
		return $this->db->get()->result();	
	}
	...
	public function join_full(array $cond)
	{
		$this->db->select("*");
		$this->db->from("peserta");
		$this->db->join("pendaftar", "peserta.no_pendaftaran = pendaftar.no_pendaftaran");
		$this->db->join("jadwal_tes", "peserta.id = jadwal_tes.id");
		if(count($cond) > 0)
			$this->db->where($cond);
		return $this->db->get()->result();	
